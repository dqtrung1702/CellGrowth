...
class RegistrationService:
    """
    Đăng ký tài khoản và (tuỳ chọn) gửi AccessRequest.
    Tách khỏi AuthService để thu hẹp trách nhiệm.
    """
    ...
    def _ids_from_codes(self, table: str, column: str, codes: List[str]) -> List[int]:
        ...
        with repo.session() as session:
            rows = session.execute(
                repo.execute(session, f"SELECT id FROM {table} WHERE {column} = ANY(:codes)", {"codes": codes})
            ).scalars().all()
            return rows
