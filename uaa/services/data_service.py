...
class DataService:
    def __init__(self, perm_repo: Optional[PermissionRepoProtocol] = None, cache: Optional[RedisCache] = None):
        self.perm_repo: PermissionRepoProtocol = perm_repo or PermissionRepository()
        # bump prefix to avoid stale cached rows with old column aliases
        self.cache = cache or RedisCache(ttl_seconds=int(Config.AUTHZ_CACHE_TTL), prefix="uaa:perm:list:v2:")

    def dataset_by_user(self, uid: int):
        rows = self.perm_repo.list_data_set_by_user(uid)
        return ResponseEnvelope(status="OK", data=rows)

    def dataset_by_permission(self, pid: int):
        rows = self.perm_repo.list_dataset_by_permission(pid)
        return ResponseEnvelope(status="OK", data=rows)
