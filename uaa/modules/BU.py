...
def updateBUbyBUId():
    if True:
        ...
        if setrcd:
            ...
            data.update({'Code':bu.Code,'LastUpdateDateTime':datetime.now(),'LastUpdateUserName':auth_info.get('UserName','???').strip().lower()}) #update Code: role.Code để chặn không cho update Code
            bu.update(data)
            res = json.dumps({"data":bu.json(),"status":"OK"},default=json_util.default).encode('utf-8')
            status = 200
        else:
