...
BEGIN
    ...
    SELECT id INTO rid_person FROM uaa.roles WHERE code = 'PERSON';
    ...
    SELECT id INTO pid_person FROM uaa.permissions WHERE code = 'PERSON' AND permission_type = 'ROLE';
    ...
    IF rid_uaa IS NOT NULL AND pid_uaa IS NOT NULL THEN
        ...
        ON CONFLICT (role_id, permission_id) DO NOTHING;
    END IF;
    IF rid_person IS NOT NULL AND pid_person IS NOT NULL THEN
        INSERT INTO uaa.role_permissions (role_id, permission_id) VALUES (rid_person, pid_person)
        ON CONFLICT (role_id, permission_id) DO NOTHING;
    END IF;
